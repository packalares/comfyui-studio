// HTTP client for the ComfyUI server. All requests go through fetchComfyUI
// which throws ComfyUIHttpError on non-2xx. Re-exports detectMediaType,
// collectNodeOutputFiles, and GalleryItem for callers that previously
// imported those from this module.
...
// ComfyUI returns `{ [promptId]: { prompt, outputs, status, ... } }` even for
// the single-id endpoint; we unwrap to the inner entry. Returns null when the
// id is unknown (history entries age out after a server restart).
