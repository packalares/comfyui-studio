...
//
// No timers, no periodic polling. We maintain a Set of promptIds we're
// "watching" (added when a prompt is submitted). When the WS relay sees
// a `queue` status update from ComfyUI, we check which watched promptIds
// are no longer in running ∪ pending — those have just finished, so we
// immediately call `appendHistoryEntry` once for each and remove them.
//
...
import { fetchComfyUI } from '../comfyui/api.js';
import { appendHistoryEntry } from './service.js';
import * as repo from '../../lib/db/gallery.repo.js';
import { logger } from '../../lib/logger.js';
import { setPromptMeta, clearPromptMeta, _clearAllPromptMetaForTests } from './promptMeta.js';
