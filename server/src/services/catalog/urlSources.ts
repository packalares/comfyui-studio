// Pure URL helpers: host detection, priority sort, dedup/merge for urlSources[].
//
// Priority is fixed by host family (hf=0, civitai=1, github=2, generic=3);
// index 0 in a sorted array is the best candidate, mirrored onto the legacy
// `url` field for backwards compatibility.
...
// Dedup by URL (existing declaredBy wins to preserve attribution), then sort.
...
/** Build a single UrlSource from a raw URL + declaredBy tag. Returns null for bad/non-http URLs. */
...
// Mutates `existing` in place. Encodes the urlSources merge + legacy-url mirror +
// `template:`/`user`/`manual` save_path overwrite + downloading/thumbnail/error/size_bytes rules.
export function mergeIntoExisting(
  existing: CatalogModel,
  entry: Partial<CatalogModel>,
): void {
  // urlSources first so the legacy `url` always reflects the new priority winner.
  if (entry.url) {
  ...
  // and template-derived upserts know the authoritative folder, so we let them
  // overwrite both. Plain seed/scan upserts keep existing values.
  ...
  // Gated state propagates (set OR clear). `entry.gated === false` is authoritative;
  // `undefined` leaves existing alone.
