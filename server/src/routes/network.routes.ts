...
const SETTERS: Record<string, SetterFn> = {
  'pip-source': (v) => {
    ...
    return s ? configurator.setPipSourceConfig(s) : bad('value must be a non-empty string');
  },
  ...
  'github-proxy': (v) => {
    const s = asNonEmptyString(v);
    return s ? configurator.setGithubProxyConfig(s) : bad('value must be a non-empty string');
  },
  'plugin-trusted-hosts': (v) => {
    const parsed = parseHostList(v);
    return parsed.ok ? configurator.setPluginTrustedHostsConfig(parsed.hosts) : bad(parsed.message);
  },
  'model-trusted-hosts': (v) => {
    const parsed = parseHostList(v);
    return parsed.ok ? configurator.setModelTrustedHostsConfig(parsed.hosts) : bad(parsed.message);
  },
  'pip-allow-private-ip': (v) => {
    if (typeof v !== 'boolean') return bad('value must be boolean');
    return configurator.setAllowPrivateIpMirrorsConfig(v);
  },
};
