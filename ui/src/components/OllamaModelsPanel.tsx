    ...
    <SelectField value={value} onValueChange={onSelect} onOpenChange={handleOpenChange}>
      ...
      <SelectContent>
        ...
        {!loading && tags && tags.length === 0 && (
          <div className="px-2 py-2 text-xs text-slate-400">No tags available.</div>
        )}
    ...
    <div className="space-y-3">
      ...
      {tab === 'huggingface' && (
        <div className="grid gap-2 md:grid-cols-2 lg:grid-cols-3">
          ...
          {hfBusy && hf.length === 0 && (
            <div className="col-span-full py-8 text-center"><Spinner size="lg" className="mx-auto text-slate-400" /></div>
          )}
            {hf.map(m => (
              <Card key={m.id}>
                <CardHeader className="flex items-start justify-between gap-3">
